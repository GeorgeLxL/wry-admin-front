...
class CreateSinglePayment extends Component {
    constructor(props) {
        super(props);
        var now = new Date();
        var d1 = new Date(new Date(now).setHours(now.getHours() + 9));
        var  day =new Date(new Date(d1).setMinutes(d1.getMinutes()+10)).toJSON().slice(0,19);
        ...
        this.state = {
            ...
            [
                {
                  label: <input type='checkbox' id='all' checked={false} onChange={this.selectAlluser}/>,
                  field: 'Checkbox'
                },
                {
                  label: "Regi.Date",
  ...
  selectAlluser = e =>{
      var {columns,data,selectAll,paymentUserid} = this.state;
      var temp = columns[0];
      temp.label = <input type='checkbox' id='all' checked={!selectAll} onChange={this.selectAlluser}/>
      columns[0] = temp;
      paymentUserid=[];
      ...
      {
        var artistlisttem = {};
        artistlisttem.ArtistId = data[i].userID;
        if(!selectAll){paymentUserid.push(artistlisttem);}
        else{paymentUserid=[];}
        data[i].Checkbox= <input type='checkbox' id={`check${data[i].userID}`} checked={!selectAll} onChange={this.changeuserId(data[i].userID)}/>;              
      }
  ...
  confirm = e => {
      ...
      if(val<=0 || ((moment(this.state.duedate) < new Date(day) && !this.state.sendcheckbox) ))
      {
          return
      }
  ...
  createPayment = e=>{
     e.preventDefault();
     const {selectAll,title,description,amount,duedate,paymentUserid,sendcheckbox,day}=this.state;
     var userData = JSON.parse(localStorage.userData);
     var token = userData.admin.token;
     var date = sendcheckbox ? day.slice(0,16).replace("T", " ") : duedate.slice(0,16).replace("T"," ");
     var data;       
     var config;
     var data;
     var multiSelect = (paymentUserid.length>1);
     if(selectAll || multiSelect)
     {
         data = JSON.stringify({"Title":title,"Description":description,"Amount":amount,"DueDate":date, "Artists":paymentUserid,"SendImmediately":sendcheckbox});
         var config = {
            method: 'post',
      ...
      {
         data = JSON.stringify({"Title":title,"Description":description,"Amount":amount,"DueDate":date,"SendImmediately":sendcheckbox});
         config = {
            method: 'post',
            
            url: `${apiurl}/payments/artists/` + paymentUserid[0].ArtistId,
            headers: { 
             'Authorization': 'Bearer ' + token,
             'Content-Type': 'application/json'
            },
     ...
     axios(config)
      .then((response)=>{
          if(this.state.selectAll || multiSelect)
          {
            var usernames="";
            ...
            this.setState({
                ...
                alertContent:<span>Payment request was successfully submitted.<br/> This may take time to complete the process.</span>
            })
          }
          else
          {
          ...
          if(this.state.selectAll || multiSelect)
          {
            ...
            else if(error.response.data.errors.DueDate)
            {
                ...
                Object.keys(errors).forEach(function (item) {
                    alertContent.push(<span style={{textAlign:'left'}}>{names[item.slice(item.indexOf("[") + 1,item.indexOf("]"))] + ": No record found"}<br/></span>);
                });
            }
            this.setState({
                paymentConfirmModal:!this.state.paymentConfirmModal,
            ...
            this.setState({
                ...
                alertModal:!this.state.alertModal,
                alertTitle:"Create Payment Failed",
                alertContent:error.response.data.errors.User?error.response.data.errors.User.message:"Date should be future"
            })
          }
  ...
  setChecbox = datafild => e=>{
    this.setState({
        [datafild]:e.target.checked,
    })
}
  ...
  changeuserId = id =>(event)=>{
    ...
    const {data,selectAll,paymentUserid} = this.state;
    if(selectAll) return;
    for(var i=0;i<data.length; i++)
    {
        if(data[i].userID == id){
            var isChecked = document.getElementById(`check${data[i].userID}`).checked;      
            var index = paymentUserid.indexOf(id);
            if(index==-1 && isChecked)
            {
                var artistlisttem = {};
                artistlisttem.ArtistId = data[i].userID;
                paymentUserid.push(artistlisttem);
                data[i].Checkbox= <input type='checkbox' id={`check${data[i].userID}`} checked={selectAll|true} onChange={this.changeuserId(data[i].userID)}/>;
            }
            else
            {
                paymentUserid.splice(index,1);
                data[i].Checkbox= <input type='checkbox' id={`check${data[i].userID}`} checked={selectAll|false} onChange={this.changeuserId(data[i].userID)}/>;
            }   
        }
    }
     this.setState({
 ...
 getdata(filter){       
    ...
    .then((response) => {
      ...
      responsedata.forEach(item => {
          var temp={};         
          temp.Checkbox = <input type='checkbox' id={`check${item.id}`} checked={false} onChange={this.changeuserId(item.id)}/>;
  ...
  render() {
    const {searchModal_user, searchResultModal_user, alertModal,alertTitle,alertContent, data, columns, pageCount, pageCountModal, createPaymentModal, paymentConfirmModal, title,description,amount,duedate,sendcheckbox,title_error,description_error,amount_error,duedate_error} = this.state;
    ...
    let hour = duedate.slice(11,13);
    let min = duedate.slice(14,16);

    let displaydueDate = `${montharray[parseInt(month-1)]} ${parseInt(day)}, ${year} ${hour}:${min}`;
    if (start === end) {
        label = "All:" +  "~" + end;
    }
    return (
        <MDBContainer className='mt-3 maincontainer'>
            ...
            <MDBModal className="createpaymodal"  isOpen={createPaymentModal} toggle={this.togglemodal("createPaymentModal")}>
                <MDBModalBody className="bg-gray-modal p-8 pr-12 pl-12">
                    ...
                    <div className="selected_user">Selected User {this.state.paymentUserid.length? this.state.paymentUserid.length : 1}</div>
                    ...
                    <MDBRow className="mb-4">
                        ...
                        <div className='col-8'>
                            <input  type='datetime-local' className='form-control' id='duedate' value={duedate} onChange={this.createPaymentdata("duedate")} placeholder='｜ Type here...' disabled={sendcheckbox}/>
                    ...
                    <MDBRow className="mb-4">
                        ...
                        <div className='col-8'>
                            <input  type='checkbox' id='sendcheckbox' name='sendcheckbox' onChange={this.setChecbox("sendcheckbox")} checked={sendcheckbox}/>
                            <label htmlFor='sendcheckbox' className='pl-5 text-lg font-semibold'>
                                Send Immediately
                            </label>
                        </div>
                    </MDBRow>
            ...
            <MDBModal className="createpaymodal"  isOpen={paymentConfirmModal} toggle={this.togglemodal("paymentConfirmModal")}>
                <MDBModalBody className="bg-gray-modal p-8 pr-12 pl-12">
                    ...
                    <MDBRow className="mb-4">
                        ...
                        <div className='col-8'>
                            <input disabled={true} type='text' className='form-control' id='title' value={title} onChange={this.createPaymentdata("title")}/>
                        </div>
                    </MDBRow>
                    <MDBRow className="mb-4">
                        <label htmlFor='description' className='col-3 col-form-label pl-10 text-lg font-semibold'>
                        ...
                        <div className='col-8'>
                            <textarea disabled={true} type='text' className='form-control' id='description' value={description} onChange={this.createPaymentdata("description")} ></textarea>
                        </div>
                    </MDBRow>
                    <MDBRow className="mb-4">
                        <label htmlFor='amount' className='col-3 col-form-label pl-10 text-lg font-semibold'>
                        ...
                        <div className='col-8 amount-input'>
                            <input  disabled={true} style={{textAlign:'right',paddingRight:'50px'}} type='text' className='form-control' id='amount' value={amount>0?amount:0.00} onChange={this.createPaymentdata("amount")}/>
                            <span className="amount-unit">USD</span>
                        </div>
                    </MDBRow>
                    <MDBRow className="mb-4">
                        ...
                        <div className='col-8'>
                            <input disabled={true} style={{textAlign:'right'}} className='form-control' id='duedate' value={displaydueDate} onChange={this.createPaymentdata("duedate")}/>
                        </div>
                    </MDBRow>
                        {sendcheckbox && (
                            <MDBRow className="mb-4">
                                <label htmlFor='checkImmediately' className='col-3 col-form-label pl-10 text-lg font-semibold'>
                                </label>
                                <div className='col-8'>
                                    Send Immediately
                                </div>
...
export default CreateSinglePayment;
